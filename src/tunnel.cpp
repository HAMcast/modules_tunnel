...
void tunnel::init()
{
    HC_LOG_TRACE("");
    m_open = true;
    m_loop = std::thread(tunnel::receive_loop_helper, this);
}
    ...
    while (true) {
        ...
        if ((ret = ::recvfrom(m_recvsock, recv_buffer, buflen-1, 0,
            ...
            case TUN_MSG_JOIN: {
                ...
                std::lock_guard<std::mutex> lg_guard(m_local_groups_mutex);
                if ((m_local_groups.count (recv_msg.group_uri()) == 0) && !(recv_msg.group_uri()).empty()) {
                    std::lock_guard<std::mutex> rg_guard(m_remote_groups_mutex);
            ...
            case TUN_MSG_JOIN_ACK: {
                ...
                std::lock_guard<std::mutex> tg_guard(m_tmp_groups_mutex);
