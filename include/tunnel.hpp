...
private:
    ...
    std::mutex                    m_local_groups_mutex;   // local grp mutex
    std::map<hamcast::uri,std::set<SA> > m_remote_groups;        // remote joined grps
    std::mutex                    m_remote_groups_mutex;   // remote grp mutex
    std::set<hamcast::uri>          m_tmp_groups;           // local tmp grps
    std::mutex                    m_tmp_groups_mutex;      // local tmp mutex
    std::thread                   m_loop;                 // receive loop thread
